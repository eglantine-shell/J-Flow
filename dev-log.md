...
## 2026-07-18（V3.2：坚果云本地文件夹同步 `.tmp` 写入兼容热修）

### 本轮目标
- 修复 V3.2 真实 Mac / Windows 双端验证中，Windows A / B 可互通但 Mac C 与 Windows 最新内容无法互通的问题。
- 重新打包 macOS DMG，并重新准备 Windows 真机打包源码交接包。

### 排查结论
- Mac 正式版同步目标为：
  - `/Users/yetingzhi/Nutstore Files/J-Flow`
- Windows 截图显示路径为：
  - `C:\Users\萧锦瑟\Nutstore\1\J-Flow`
- 进一步对照后，`\1\J-Flow` 更像坚果云 Windows 客户端的本地映射路径，并非必然是错误目录：
  - Windows 资源管理器面包屑显示为 `坚果云 > J-Flow`
  - 根目录中的 `sync-info-NSConflict...` 文件列表与 Mac 侧高度一致
- 真实异常是 Windows 侧同步目录中出现：
  - `sync-info.json.<timestamp>-<random>.tmp`
  - 且状态同时显示绿色勾与红叉
- Mac 侧未收到 Windows 设备 `59a28fdf-1264-4059-8099-520d850d1934` 的 2026-07-17 / 2026-07-18 更新，仍停在 2026-07-08。
- 此时曾判断当前 `.tmp -> rename` 原子写法与坚果云目录监听 / 上传机制存在兼容风险，可能导致 Windows 本地显示成功但跨平台落地不一致。
- 后续使用普通 txt 探针继续验证：
  - Windows 端创建 `jflow-sync-probe-win.txt`
  - 坚果云网页端可见该文件
  - Mac 本地 `/Users/yetingzhi/Nutstore Files/J-Flow` 搜不到该文件
- 最终人工确认：
  - Mac 自动更新后坚果云客户端被关闭，且没有开机自启
  - 本次“双端完全不同步”的最终根因是 Mac 坚果云客户端未运行
  - J-Flow 代码层不是最终根因
  - 本轮 `.tmp` 写入修复仍作为第三方云盘目录兼容性改进保留

### 本轮修改
- `electron/sync-target/local-folder-driver.ts`
  - `safeWriteJson` 不再在目标同步目录内创建 `.tmp` 临时文件再 `rename`
  - 改为直接写最终 JSON 文件
  - 保留原有 logical path 安全校验、自动创建父目录、JSON pretty print
- `electron/sync-target/local-folder-driver.test.ts`
  - 新增回归测试：`safeWriteJson` 后 `devices/` 中只存在最终 `.json`，不留下 `.tmp`

### 设计取舍
- 对普通本地磁盘，`.tmp -> rename` 更接近原子写。
- 但当前同步目标常位于坚果云等第三方云盘目录，小 JSON 文件快速 `.tmp -> rename` 更容易被云盘客户端捕捉为异常中间态。
- 当前同步 JSON 文件很小，直接覆盖最终文件更适合本项目的第三方云盘目录使用场景。
- 导入端继续只扫描 `.json` 文件，并对单个坏 JSON 记录 failure，不中止扫描其余文件。
...
- 后续使用坚果云等第三方云盘目录作为同步文件夹时，应先确认两端云盘客户端正在运行并完成目标文件夹同步。
- 若再次出现跨端不同步，优先用普通 txt 探针确认第三方云盘跨端落地，再排查 J-Flow 导入 / 导出。

## 2026-07-17（V3.2：Mac / Windows 同步 updatedAt 相等热修）

### 本轮目标
- 暂停 release 更新。
- 排查 Win A / Win B 能互通，但 Win A 最新内容与 Mac C 最新内容无法互通的问题。

### 根因
- Win A / Win B 能互通，说明同步目录、Windows 包、基础导入导出链路可用。
- Mac C 与 Win A 无法互通的关键原因是：
  - 同一实体两端业务 `updatedAt` 相等
  - 但 V3/V3.1 migration / repair 后实体内容不同
  - `electron/sync-import.ts` 只在 `remote.updatedAt > local.updatedAt` 时应用远端
  - 相等时直接跳过，导致两端都认为对方“不更新”
- V3.2 repair 此前重新排队时沿用业务实体 `updatedAt`，导出的 sync item 顶层 `updatedAt` 也来自业务实体，因此无法打破“同时间戳不同内容”的僵局。

### 本轮修改
- `electron/sync-export.ts`
  - sync item 新增 `syncUpdatedAt`
  - 普通导出时 `syncUpdatedAt` 与业务 `updatedAt` 一致
  - repair 导出时 `syncUpdatedAt` 可晚于业务 `updatedAt`
- `electron/sqlite.ts`
  - V3.2 repair 的 upsert `sync_changes.changedAt` 改为 repair 排队时间
  - 不改写业务实体 `updatedAt`
  - delete tombstone 仍保持原删除时间，避免旧删除误伤较新远端实体
- `electron/sync-import.ts`
  - 远端业务 `updatedAt > local.updatedAt`：继续应用远端
  - 远端业务 `updatedAt < local.updatedAt`：继续跳过远端
  - 远端业务 `updatedAt === local.updatedAt`：若实体内容不同，则应用当前同步文件内容
- `electron/types.ts`
  - `SyncItemFile` 增加可选 `syncUpdatedAt`
- 测试：
  - 导出 sync item 包含 `syncUpdatedAt`
  - 远端与本地 `updatedAt` 相等但内容不同会应用远端
  - V3.2 repair upsert 刷新同步排队时间但不改业务 `updatedAt`
...
- 已重新打包 macOS DMG：
  - `release/J-Flow-V3.2.dmg`
  - SHA256：`e8063e24520a3addb91987dafbd79770b76743c082c62786b9602b90d0f10cab`
  - `hdiutil imageinfo`：通过
  - `hdiutil attach`：通过
  - `J-Flow.app` 版本为 `3.2.0`
  - `icon.icns / icon.png` 存在
...
- 当前 `release/J-Flow-V3.2-win-portable.exe` 与 `release/J-Flow-V3.2-win-setup.exe` 仍是热修前 Windows 包，不包含本热修。
- 下一步需要使用新的源码交接包重新打包 Windows 端。

## 2026-07-17（V3.2：Windows 真机打包产物归档）

### 本轮目标
- 接收 Windows 端 V3.2 打包结果。
- 将 Windows portable / setup exe 放入 Mac 工作区 `release/`。
- 核对 Windows exe SHA256。
- 更新 README / handoff / dev-log / task-list 的 V3.2 Windows 状态。
...
- `J-Flow-V3.2-win-portable.exe`
  - Windows 回传：`B6D04629275C9B31D98A68767DD46E291A5436AB3D2EB0FA0C770A6E054C4B6A`
  - Mac 本地核对：`b6d04629275c9b31d98a68767dd46e291a5436ab3d2eb0fa0c770a6e054c4b6a`
- `J-Flow-V3.2-win-setup.exe`
  - Windows 回传：`2DFBBEF8A6516907F0EABAD47A9EDB9A31ECBFC8E13B840744428F8FFB570ABD`
  - Mac 本地核对：`2dfbbef8a6516907f0eabad47a9edb9a31ecbfc8e13b840744428f8ffb570abd`
...
### Windows 侧验证
- 源码 zip SHA256 已匹配：
  - `050318bec614394bbd74c6ad30aa96f8ea023d7a0ad8cf9351a4fe4eebc3e491`
...
- `pnpm install` 在 Electron postinstall 出现 `ECONNRESET`
  - 已复用本机缓存的 Electron `41.3.0` Windows x64 runtime 手动补齐
- 没有复现 V3.1 的 WindowsApps `node` 权限问题
  - 本轮继续使用 `E:\J-Flow\.pnpm-tools` 里的本地 `node / pnpm` shim
- `sync:icon` 默认覆盖 `build/icon.png` 时遇到权限拒绝
  - Windows 侧改临时 PNG 输出路径生成 `build/icon.ico`
  - 最终 `build/` 只保留 `icon.png` 和 `icon.ico`
- Windows 侧为 SQLite 测试稳定通过做了最小测试兼容修复：
  - 新增关闭 cached SQLite 连接的测试 helper
  - 在相关测试清理临时目录前调用
  - 避免 Windows 因 sqlite/wal/shm 句柄未释放报 `EBUSY`
...
## 2026-07-17（V3.2：macOS DMG 打包与 Windows 源码交接包）

### 本轮目标
- 将 V3.2 修复版打包为 macOS DMG，供真实数据验证。
- 准备 Windows 端打包 exe 所需源码交接 zip。
- 更新 README / handoff / dev-log 中的 V3.2 打包状态。

### 本轮修改
- `package.json`
  - `version` 更新为 `3.2.0`
  - macOS DMG artifact 更新为 `J-Flow-V3.2.dmg`
  - Windows portable artifact 更新为 `J-Flow-V3.2-win-portable.exe`
  - Windows setup artifact 更新为 `J-Flow-V3.2-win-setup.exe`
- `README.md`
  - 当前桌面版产物更新为 V3.2
  - 补充 V3.2 macOS SHA256
  - 当时标注 V3.2 Windows 包待真机打包；后续已完成，详见上方 Windows 真机打包产物归档记录
...
### DMG 校验
- `hdiutil imageinfo release/J-Flow-V3.2.dmg`：通过
- `hdiutil attach -nobrowse -mountpoint /private/tmp/jflow_v32_dmg_mount release/J-Flow-V3.2.dmg`：通过
- DMG 内：
  - `J-Flow.app`
  - `CFBundleDisplayName = J-Flow`
  - `CFBundleShortVersionString = 3.2.0`
  - `CFBundleIconFile = icon.icns`
  - `Contents/Resources/icon.icns` 存在
  - `Contents/Resources/icon.png` 存在
- 挂载点已卸载。
...
- 注意：
  - `build/` 当前只含 `icon.png`
  - Windows 端打包前需要先运行 `corepack pnpm run sync:icon` 生成 `build/icon.ico`

### 打包输出提示
- Vite chunk size warning：既有非阻塞提示。
- electron-builder 提示 `package.json` 缺 `description / author`：非阻塞。
- macOS notarization skipped：当前未签名 / 未公证，符合既有状态。
- macOS 打包期间提示 `build/icon.ico` 不存在：对 macOS DMG 不阻塞；Windows 端需通过 `sync:icon` 生成。

## 2026-07-17（V3.2：同步 repair 与分步日志快照修复）

### 本轮目标
- 排查 V3 后 Mac / Windows 双端同步异常。
- 修复 V3.1 分步 Todo 在日志快照中只显示基础事项、不显示当前步骤的问题。

### 排查结论
- 同步异常不是同步按钮或同步目标规则被改动导致。
- V3 修复重复事项日夜归属时新增了 `TaskTemplate.timeBlock / timeBlockSource`。
- V3.1 分步优化新增了 `plannedSteps`。
- SQLite migration 会本地补列与回填，但此前没有把受模型升级影响的旧 `taskTemplate / dayPlanItem` 重新放回 `sync_changes`。
- 因此旧数据升级后可能只在本机被修好，不会自然重新发布到同步文件夹。
- 日志快照分为两条生成路径：
  - renderer 日志服务已使用分步显示标题
  - Electron 自动补昨日日志仍使用原始 `item.title`

### 本轮修改
- 日志：
  - `electron/daily-logbook.ts` 新增分步显示标题逻辑
  - completed / pending / deleted 快照均保存 `事项：当前步骤`
  - `src/features/todo/TodoModePanel.tsx` 中分次进度日志标题快照也改用同一显示口径
- 同步：
  - `sync_meta` 新增一次性标记 `v32SyncRepairAppliedAt`
  - 启动 / 打开 SQLite 后，若已有业务数据且未执行过 V3.2 repair，则将当前库中的 `taskTemplate / dayPlanItem` 重新加入待同步队列
  - repair 不改写业务实体 `updatedAt`
  - 远端旧分步 item 缺 `plannedSteps` 时，从 `nextStep` 恢复后续步骤队列
...
### 后续注意
- V3.2 repair 需要真实双端同步场景人工验证：
  - 一端升级后执行“立即同步”
  - 另一端升级后执行“立即同步”
  - 确认重复事项夜晚归属、分步后续步骤队列、普通新增 / 删除仍能双端交换
- 若用户已在某一端使用旧 V3/V3.1 产生了较新的实体，本轮同步仍遵循现有 LWW 比较，repair 本身不提高业务实体 `updatedAt`。

## 2026-07-08（V3.1：Windows 真机打包完成并删除源码交接包）

### 本轮目标
- 接收 Windows Codex 打包结果。
- 删除 Mac 工作区中的 Windows 源码交接 zip。
- 核对 Windows exe SHA256。
- 更新 README / handoff / dev-log / task-list 的 Windows 状态。
...
- Mac 本地 `shasum -a 256` 核对结果与 Windows 交接说明一致。
...
### Windows 构建注意事项
- Windows 机器 PATH 中的 `node` 会被 WindowsApps 权限挡住。
...
- 后续如果重新安装依赖，最好确保 Windows 侧 PATH 先指向可用 Node，或者复用 `.pnpm-tools` 方案。

### 非阻塞提示
- Vite chunk size warning。
- electron-builder 提示 `package.json` 缺 `description / author`。

## 2026-07-08（V3.1：Windows 源码交接包）

### 本轮目标
- 为后续 Windows 真机打包准备源码交接包。
- 确认交接包不包含 macOS release 产物、本机依赖、构建输出或缓存。
- 准备 git 更新命令。
...
- 本轮仅生成源码交接包，未在 macOS 本机生成 Windows exe。

## 2026-07-08（V3.1：测试收口、README 更新与 macOS DMG 打包）

### 本轮目标
- 跑 TypeScript、SQLite / storage 与分步相关回归测试。
- 核实并更新 README。
- 将版本与打包产物切到 V3.1。
- 生成并校验 V3.1 macOS DMG。

### 测试结果
- `corepack pnpm exec tsc --noEmit`：通过。
- `corepack pnpm exec tsc -p electron/tsconfig.json --noEmit`：通过。
- `corepack pnpm exec vitest run src/db/storage.test.ts electron/sqlite.test.ts electron/selected-date-state.test.ts`：通过。
  - `3` 个测试文件通过。
  - `23` 个测试项通过。

### 分步回归覆盖
- `electron/sqlite.test.ts` 覆盖：
  - `TaskTemplate.plannedSteps` 写入与读取。
  - `DayPlanItem.plannedSteps` 写入、更新与读取。
  - 旧 `nextStep` 数据标准化为 `plannedSteps`。
- `electron/selected-date-state.test.ts` 保持 selected-date 生成路径既有回归通过。
- `src/db/storage.test.ts` 保持 storage schema / migration 相关回归通过。

### README 与版本
- 更新 [README.md](./README.md)：
  - `V3.1` 从“计划优化”改为“已完成分步 Todo 优化”。
  - 当前桌面产物改为：
    - `J-Flow-V3.1.dmg`
    - `J-Flow-V3.1-win-setup.exe`
    - `J-Flow-V3.1-win-portable.exe`
...
- 更新 [package.json](./package.json)：
  - `version = 3.1.0`
  - `dmg.artifactName = J-Flow-V3.1.dmg`
  - `win.artifactName = J-Flow-V3.1-win-portable.${ext}`
  - `nsis.artifactName = J-Flow-V3.1-win-setup.${ext}`
...
- `release/latest-mac.yml` 已指向：
  - `version: 3.1.0`
  - `path: J-Flow-V3.1.dmg`
- 挂载 `release/J-Flow-V3.1.dmg` 校验：
  - `CFBundleDisplayName = J-Flow`
...
### 当前注意
- `package:mac` 仍提示 `build/icon.ico` 不存在；这是 Windows extraResources 提示，不影响 macOS DMG。
- `build:desktop` 仍有既有 Vite 大 chunk warning，不阻断。
- V3.1 Windows 包尚未打包，仍需 Windows 真机完成。

## 2026-07-08（V3：暂停 Windows 交接，评估多步骤分步 Todo）

### 本轮目标
- 暂停 Windows 打包与交接。
- 删除已生成的 Windows 源码交接包。
- 评估 `分步 Todo` 从单个下一步扩展为多个预排步骤的方案。
- 用户确认该需求计为 `V3.1 分步优化`。

### 本轮处理
- 已删除：
  - `J-Flow-V3-win-handoff-source-20260707.zip`
- 当前不继续生成新的 Windows 交接包。
- Windows 交接需要等多步骤分步 Todo 完成、验证、文档更新后再重做。
...
- 现有分步模型为：
  ...
  - `stepRootItemId`
  - `previousStepItemId`
- 现有完成逻辑只会读取单个 `nextStep` 并创建下一条 Todo。
- 新需求需要表达有序后续步骤队列，建议新增兼容字段：
  - `plannedSteps: string[]`
- 旧数据兼容建议：
  - 若 `plannedSteps` 缺失且 `nextStep` 非空，则读取时视为 `[nextStep]`
  - 保存新数据时写入 `plannedSteps`
  - `nextStep` 可在兼容期继续保存为 `plannedSteps[0] ?? ''`
- 自动创建下一步时：
  - 新 Todo 的 `currentStep` 取当前队列第一项
  - 新 Todo 的 `plannedSteps` 取剩余队列
  - 新 Todo 的 `nextStep` 兼容写为剩余队列第一项或空
  - 继续不继承 DDL，DDL 仍只属于当前步骤

### 文档更新
- 更新 `product-rules.md`：
  - 将分步 Todo 规则扩展为多个后续步骤队列
  - 记录 `+` 按钮增加后续步骤输入行
  - 明确旧 `nextStep` 兼容策略
  - 记录教学需要同步展示多个后续步骤
- 更新 `data-model.md`：
  - 建议新增 `plannedSteps: string[]`
  - 计划 JSON app data schema 升级到 `14`
  - 计划 SQLite schema 升级到 `8`
  - 计划新增 `planned_steps_json` 存储列
...
- 更新 `README.md`：
  - 分步说明改为支持多个后续步骤
  - 版本线补充 `V3.1`
- 更新 `task-list.md`、`handoff.md`：
  - 新增当前发布口径的 `V3.1 分步 Todo 优化`
  - 明确 Windows 交接待 V3.1 完成后重做
...
- 更新 JSON schema：
  - `APP_DATA_SCHEMA_VERSION = 14`
  - `taskTemplateSchema.plannedSteps`
  - `dayPlanItemSchema.plannedSteps`
- 更新 Web / renderer 存储标准化：
  - 旧数据若只有 `nextStep`，读取时补为单元素 `plannedSteps`
  - 保存新数据时继续同步 `nextStep = plannedSteps[0] ?? ''`
- 更新 SQLite：
  ...
  - SQLite map / insert / update / full snapshot 路径均支持 `plannedSteps`
- 更新重复生成的数据模型继承：
  - Web / renderer recurrence 生成路径继承模板 `plannedSteps`
  - Electron selected-date 生成路径继承模板 `plannedSteps`
- 更新测试：
  - SQLite 多步骤 `plannedSteps` 往返
  - 旧 `nextStep` 标准化为 `plannedSteps`
...
- Todo 表单已改为多个后续步骤输入行。
- 分步完成时的“队列弹出”逻辑已实现：
  - 当前 Todo 完成后，取 `plannedSteps[0]` 创建下一步 Todo
  - 新 Todo 的 `plannedSteps` 取原队列剩余项
  - 新 Todo 的 `nextStep` 兼容写为剩余队列第一项或空
  - 若旧数据只有 `nextStep`，仍可按单后续步骤生成
- 教学 demo 与 Guide 文案已进入代码实现。
- Windows 交接包仍暂停，待 V3.1 完整实现后重做。

### Todo 表单与完成逻辑实现
- 更新 `src/features/todo/TodoModePanel.tsx`：
  - 将单个 `nextStepDraft` 改为 `plannedStepDrafts`
  - 分步表单中后续步骤区域支持多行输入
  - 第一行后续步骤右侧显示 `+` 按钮
  - 后续新增行右侧显示删除按钮
  - 保存时过滤空白后续步骤，并写入 `plannedSteps`
  - 所有创建 / 编辑路径继续同步写入兼容字段 `nextStep`
  - 覆盖路径包括普通 Todo、重复 Todo、从种草加入 Todo、编辑 Todo
- 更新 `src/features/decision/recommendation.ts`：
  - 从种草加入 Todo 时支持接收并保存 `plannedSteps`
- 更新 `src/styles/globals.css`：
  - 新增多行后续步骤输入区与图标按钮样式
- 更新 `src/features/tutorial/TutorialOverlay.tsx`：
  - Guide “分步和分次”文案改为说明分步可一次安排多个后续步骤
  - 说明完成当前步骤后会按队列自动生成下一步
- 更新 `src/features/tutorial/tutorial-demo-data.ts`：
  - 所有 `TaskTemplate` / `DayPlanItem` demo 对象补齐 `plannedSteps`
  - `读《夜航西飞》` 分步示例改为：
    - 当前步骤：`第一章`
    - 后续步骤：`第二章 / 第三章 / 第四章`
...
- `build:desktop` 仍有既有 Vite 大 chunk warning，不阻断
- 教学 demo / Guide 更新后补充验证：
  - `corepack pnpm exec tsc --noEmit`：通过
  - `corepack pnpm exec tsc -p electron/tsconfig.json --noEmit`：通过
  - `corepack pnpm run build:desktop`：通过

## 2026-07-07（V3：release 清理与 Windows 交接准备）

### 本轮目标
- 清理本地 `release/` 中过期历史产物。
- 准备 V3 Windows 真机打包交接。
- 明确 Windows 图标生成与打包命令。

### 本轮处理
- 清理 `release/`：
  - 删除 V1.4、V2.1、V2.2、V2.3.1、V2.3.2、V2.4 的历史 DMG / EXE / blockmap
  - 删除旧的 `release/mac-arm64/`
  - 删除旧的 builder 临时配置文件
- 当前 `release/` 仅保留：
...
- 已生成 Windows 源码交接包：
  - `J-Flow-V3-win-handoff-source-20260707.zip`
- 交接包大小约 `3.0M`，包含 `181` 个文件。
- 已抽检交接包：
  - 未包含 `node_modules/`
  - 未包含 `.git/`
  - 未包含 `release/`
  - 未包含 `dist/`、`dist-desktop/`、`dist-electron/`
  - 未包含 `.env.local`、`.DS_Store`
  - 未包含 DMG / EXE / blockmap
- 交接包包含 Windows 图标与打包所需文件：
  - `J-Flow.PNG`
  - `build/icon.png`
  ...
  - `package.json`
- Windows 侧需要在 Windows 真机运行 `corepack pnpm run sync:icon`，该脚本会调用 `scripts/generate-icon-win.ps1` 生成：
  - `build/icon.png`
  - `build/icon.ico`
- `scripts/fix-win-exe-icon.mjs` 会在 Windows `afterPack` 阶段检查 `build/icon.ico` 并用本地 `rcedit.exe` 写入 `J-Flow.exe` 图标；若缺少 `icon.ico` 会直接报错，避免再次产出无图标 Windows 包。
- macOS 当前只会生成 / 保留 `build/icon.png`，因此在 macOS 上看到 `build/icon.ico` 缺失是预期现象，不代表 Windows 交接包缺文件。
...
### 后续注意
- V3 Windows 包尚未在本轮 macOS 环境真机验证。
- Windows 侧完成后，应回传：
  - `release/J-Flow-V3-win-portable.exe`
  - `release/J-Flow-V3-win-setup.exe`
  - 如生成 setup blockmap，也一并回传并记录校验信息
- 普通用户仍建议优先使用 setup，portable 适合测试和临时分发。

## 2026-07-07（V3：版本口径校正、README 更新与打包准备）

### 本轮目标
- 将原计划中的 `V2.4` 口径校正为 `V3`。
- 将原 V2.4 大更新和本轮重复 Todo 日夜归属 bug 修复统一计入 V3。
- 更新 README 与打包文件名。
- 重新打包 macOS V3 DMG，并准备 Git 命令。
...
- 更新 `README.md`：
  ...
  - 增加 V3 版本线说明
  - 将“准备”说明更新为“备注”
  - 补充分步 Todo、真实 UI 使用教学、拔草删除语义和重复 Todo 日夜归属说明
  - 标注 V3 Windows 包待单独打包验证
- 更新 `package.json`：
  - macOS DMG artifact 改为 `J-Flow-V3.dmg`
  - Windows setup artifact 改为 `J-Flow-V3-win-setup.exe`
  - Windows portable artifact 改为 `J-Flow-V3-win-portable.exe`
...
## 2026-07-07（V3：重复 Todo 日夜归属旧逻辑残留修复）

### 本轮目标
- 修复“每日重复的必要事项无法稳定放在晚上”的问题。
- 清理重复 Todo 中通过场景 tag 名称推断白天 / 晚上的旧逻辑残留。

### 排查结论
- 产品规则与手动验证清单已明确：
  - 场景 tag 不应自动映射白天 / 晚上
  - 系统不应根据 tag 中文名称做语义匹配
- 代码中仍有旧逻辑残留：
  - `src/features/recurrence/auto-generated.ts` 根据 `sceneTagIds` 和 tag 名称推断 `timeBlock`
  - `electron/selected-date-state.ts` 复制了一套同样推断逻辑
  - `src/features/todo/TodoModePanel.tsx` 创建 / 编辑重复 Todo 时按当前日夜反向写入场景 tag
- 根因是 `TaskTemplate` 未保存 `timeBlock / timeBlockSource`，导致后续 occurrence 生成时只能退回旧的场景推断。

### 本轮修改
- 更新数据模型：
  - `TaskTemplate` 新增 `timeBlock`
  - `TaskTemplate` 新增 `timeBlockSource`
  - JSON app data schema version 从 `12` 升级到 `13`
  - SQLite schema version 从 `6` 升级到 `7`
- 更新 SQLite：
  - `task_templates` 新增 `time_block`
  - `task_templates` 新增 `time_block_source`
  - 旧库 migration 优先从已有 `day_plan_items` 最近实例回填重复模板的 `time_block`
  - 若无法回填则默认 `day / default_day`
  - `replaceSqliteSnapshot`、常规 create / update、远端 apply 路径均支持新字段并对旧对象兜底
- 更新重复生成：
  - `src/features/recurrence/auto-generated.ts` 直接继承模板 `timeBlock / timeBlockSource`
  - `electron/selected-date-state.ts` 直接继承模板 `timeBlock / timeBlockSource`
  - 删除生成路径中的 tag 名称日夜推断
- 更新 Todo 表单：
  - 新建 / 编辑重复 Todo 时写入模板 `timeBlock / timeBlockSource`
  - 不再根据日夜选择自动写入“白天 / 晚上”场景 tag
  - 从种草条目转为重复 Todo 时，保留原种草场景 tag 作为普通分类元数据，但不参与日夜推断
- 更新文档：
  - `product-rules.md`
  - `data-model.md`
...
- 新增回归测试：
  - 每日重复 + 必要 + 晚上模板，下一天 occurrence 仍生成到晚上
  - 即使带有 `工作日晚上` 场景 tag，只要模板日夜为白天，下一天 occurrence 仍生成到白天

### 后续注意
- `mapped_day / mapped_night` 仍保留在枚举和旧 mock 数据中，用于兼容旧 `DayPlanItem` 数据；不再作为重复模板生成日夜的依据。
- 若后续希望彻底删除 `mapped_day / mapped_night`，需要另起迁移任务清理历史数据与日志 / mock / schema。

## 2026-07-02（V2.4 DMG：图标缺失排查与重打包）

### 本轮目标
- 检查新打包的 `J-Flow-V2.4.dmg` 没有 J-Flow 图标的问题。
- 修复 V2.4 macOS DMG 产物，确保最终 DMG 内为完整 `J-Flow.app`。

### 排查结论
- 先前的 V2.4 DMG 是在完整 `electron-builder --mac dmg` 失败后，继续使用 `release/mac-arm64` 作为 `--prepackaged` 输入生成的。
- 当时 `release/mac-arm64` 中实际是未组装完成的裸 `Electron.app`：
  - `CFBundleDisplayName` 仍为 `Electron`
  - `CFBundleIconFile` 仍为 `electron.icns`
  - `Contents/Resources` 中缺少 J-Flow 的 `dist-electron / dist-desktop / app package`
- 因此 DMG 没有 J-Flow 应用图标，也不是可作为 V2.4 正式试用的正确包。

### 本轮处理
- 在临时目录重新组装 `J-Flow.app`：
  - 从 Electron runtime 复制 app bundle
  - 写入 `dist-electron`、`dist-desktop`、`package.json`
  - 补入运行时依赖 `fast-xml-parser` 及其依赖
  - 写入 `icon.icns` 与托盘所需 `icon.png`
  - 将 `Info.plist` 改为 `J-Flow / com.jflow.desktop / 2.4.0`
- 使用修正后的 `J-Flow.app` 重新生成：
  - `release/J-Flow-V2.4.dmg`
  - `release/J-Flow-V2.4.dmg.blockmap`
...
- 挂载最终 DMG 后验证：
  ...
  - DMG 内 `J-Flow.app/Contents/Resources/icon.icns` 为有效 macOS icon
  - DMG 内包含 `dist-electron`、`dist-desktop`、`node_modules`

### 后续注意
- 当前完整 `electron-builder --mac dmg` 路径仍会在 pnpm 依赖收集阶段触发 `ERR_SQLITE_ERROR`，本轮使用手工组装 app bundle + `--prepackaged` 绕过。
- 后续应单独修复 electron-builder 与 pnpm 依赖收集兼容问题，避免每次打包都需要手动组装。
...
- 重点确认：
  ...
  的 `MKCOL 503`
## 2026-07-08 - V3.1 教学态新增 Todo 展示与数据隔离修复

### 本轮目标

- 修复教学第 3 步开始展示新增 Todo 时，Todo 新增表单偶发没有出现的问题
- 修复教学态演示 Todo `读《夜航西飞》` 可能写入真实 Todo 数据的问题
- 复测后补充修复：教学 demo 日期不应写死，应以打开教学时的本地日期动态计算

### 根因

- 教学态存储隔离此前主要依赖 URL 参数 `?tutorial=1`
- 真实 UI 教学会在路由、overlay、真实表单事件之间来回切换，只依赖 URL 参数不够稳
- Todo 新增表单存在“点外部自动保存”逻辑，教学态真实 UI 可点击时可能把演示标题作为真实新增保存
- Guide 第 3 步打开 Todo 表单依赖一次性自定义事件，若 Todo 页面组件尚未挂载或渲染未完成，事件可能被错过
- Guide 第 3 步持续不显示的实际原因是：
  - demo 选中日期写死为 `2026-07-05`
  - 该日期在 2026-07-02 开发时是未来日期
  - 到 2026-07-08 已成为历史日期
  - Todo 真实业务规则禁止历史日期新增，因此新增表单被渲染条件挡住

### 已修复

- 在 [src/vite-env.d.ts](./src/vite-env.d.ts) 中新增 renderer 会话标记：
  - `window.__JFLOW_TUTORIAL_MODE__`
- 在 [src/db/storage.ts](./src/db/storage.ts) 中让教学存储判断同时识别：
  - URL `?tutorial=1`
  - `window.__JFLOW_TUTORIAL_MODE__`
- 在 [src/features/tutorial/TutorialOverlay.tsx](./src/features/tutorial/TutorialOverlay.tsx) 中：
  - 教学开启时设置会话标记
  - 教学关闭 / unmount 时清除会话标记
  - 跳过 / 完成教学前先关闭 Todo 与种草新增表单
  - `beforeShow` 事件改为多次补发，增强真实 UI 挂载后的接收稳定性
- 在 [src/features/todo/TodoModePanel.tsx](./src/features/todo/TodoModePanel.tsx) 中：
  - 增加教学态识别 helper
  - 教学态下禁用新增 Todo 表单的“点外部自动保存”
- 在 [src/features/tutorial/tutorial-demo-data.ts](./src/features/tutorial/tutorial-demo-data.ts) 中：
  - 新增动态日期 helper
  - 教学选中日期改为“本地今天 + 3 天”
  - demo Todo / 重复实例 / 日志 / 分次记录同步使用动态日期
- 在 [src/features/tutorial/TutorialOverlay.tsx](./src/features/tutorial/TutorialOverlay.tsx) 中：
  - `jflow:tutorial-select-date` 改为发送动态教学日期
- 在 [src/db/storage.ts](./src/db/storage.ts) 中：
  - 教学内存数据按动态教学日期初始化
  - 若跨天导致动态教学日期变化，会重建 demo 数据

### 保留规则

- 本轮没有放开真实使用中的历史日期新增 Todo 限制
- 第 3 步显示问题通过把教学 demo 日期保持为未来日期解决
...
- `build:desktop` 仍有既有 Vite 大 chunk warning，不阻断
- 仍需用户在 Electron dev 版中人工确认：
  - Guide 第 3 步 Todo 表单稳定出现
  - 教学态操作和跳过后，真实 Todo 不再出现演示条目
