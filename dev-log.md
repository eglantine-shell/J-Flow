...
## 2026-04-26（V2-8：分次跨天延续与进度继承最小可用版本）

### 本轮目标
- 实现分次跨天延续与进度继承的最小可用版本
- 允许修改数据模型、schema、storage、同步逻辑与必要 UI
- 不改 recurrence 语义，不做大规模 UI polish
...
### 本轮关键判断
- `manual_temporary` 没有 `templateId`，因此必须在 `DayPlanItem` 上补实例链字段，不能靠标题推断。
- continuation 不能塞进 `TodoModePanel` 渲染期临时计算，更适合作为独立同步模块。
- 对带 `recurringInstanceId` 的 `auto_generated` 分次事项，可以安全支持“同周期内 continuation”，但不能让它打破 recurrence 新周期语义。
...
- 只延续：
  ...
  - `date < selectedDate`
- 删除某一天 continuation 只跳过当天，不结束整条链。
- 从种草重新加入分次事项时，按同 `templateId` 最近未完成链继承进度。
- recurrence 新周期不继承旧周期进度：
  - `daily` 也同样不滚动继承前一天

### 本轮修改
- 更新 `src/types/models.ts`
  - 新增 `DayPlanItem` 分次链字段
- 更新 `src/db/schema.ts`
  - 为新字段补 schema 兼容
  - schema version 升到 `3`
- 更新 `src/db/storage.ts`
  - 补 `DayPlanItem` 新字段的存储归一化
- 新增 `src/features/continuation/segmented-continuation.ts`
  - 实现 continuation 同步与模板分次链查找
- 新增 `src/features/continuation/index.ts`
- 更新 `src/features/todo/TodoModePanel.tsx`
  - selectedDate 加载时先同步 auto-generated，再同步 continuation
  - 新建分次事项后补写 `rootItemId`
  - 增加最小“延续自 x/x”提示
- 更新 `src/features/decision/recommendation.ts`
  - 从种草重新加入分次事项时继承最近未完成链进度
- 更新 `src/features/recurrence/auto-generated.ts`
  - 新生成的分次自动实例直接写入 `rootItemId`
- 更新 `src/styles/globals.css`
  - 补 continuation 小字提示样式
- 更新文档：
  - `data-model.md`
  - `product-rules.md`
  - `app-structure.md`
...
### 本轮刻意未做
- 未改 recurrence 语义
- 未让 daily recurrence 滚动继承前一天进度
- 未做临时重复新功能
- 未做 `decision_selected` 转重复
- 未引入新依赖
- 未做独立 chain 表

### 当前风险与待确认问题
- 若历史数据里出现 `recurringInstanceId` 指向缺失实例，当前实现会保守地停止该条重复分次事项的 continuation。
- 分次事项的“取消完成并回退进度”仍没有专门状态机；本轮只保证 `0%` 不会继续延续。
- continuation 当前只做最小提示，没有补完整历史链路浏览。
...
## 2026-04-26（分次跨天延续与进度继承：规则与建模方案）

### 本轮目标
- 只补分次跨天延续与进度继承的产品规则、数据建模与任务拆分
- 不改源码实现
- 允许同步规则文档、模型文档与交接文档
...
### 本轮关键判断
- 不能靠 `title` 猜测分次链，因为用户可重名，也可编辑标题。
- `manual_temporary` 没有 `templateId`，因此如果要支持跨天延续与进度继承，必须有实例链路字段。
- 相比新增独立 chain 表，先把链路字段挂在 `DayPlanItem` 上更适合当前本地单人应用，也更利于后续 iOS 迁移时做轻量映射。
- continuation 不应放进 `TodoModePanel` 组件里临时扫描；更适合独立成业务同步模块。
- 对已受 `RecurringTaskInstance` 管控的重复事项，不建议贸然打破现有“旧周期未完成记为 `expired`，不滚入新周期”的规则。

### 本轮关键决策
- 推荐采用 `DayPlanItem` 链路字段方案，而不是：
  - 独立 `SegmentedTaskChain` 表
  - 仅靠 `templateId / title / date` 推断
- 推荐新增字段：
  - `rootItemId`
  - `continuationOfItemId`
  - `carriedFromDate`
- 推荐只自动延续 `progressPercent = 1-99` 的分次事项。
- `0%` 不延续，`100%` 结束链路。
- 删除某天延续实例只跳过当天，不关闭整条链。
- 重新从种草加入同一 `templateId` 的分次事项时，推荐继承最近未完成分次链。
- 推荐同步顺序：
  1. `syncAutoGeneratedDayPlanForDate`
  2. `syncSegmentedContinuationsForDate`
  3. 读取并展示 `DayPlanItem`
...
- 更新 `product-rules.md`
  - 补充分次跨天延续与进度继承规则草案
- 更新 `data-model.md`
  - 补充 `DayPlanItem` 链路字段方案与兼容策略
- 更新 `app-structure.md`
  - 补充 continuation 独立模块与加载顺序建议
- 更新 `task-list.md`
  - 新增 V2-8：分次跨天延续与进度继承
- 更新 `handoff.md`
  - 同步下一阶段重点与推荐实现方向
...
- 未改 storage
- 未改 Todo / Home 加载链路
- 未改 `RecurringTaskInstance` 数据结构
- 未实现跨天延续
- 未实现进度继承

### 当前风险与待确认问题
- 若后续要让“日历型重复 + 分次事项”在新周期继续沿用旧进度，会直接冲撞现有 `expired` 规则，这一点必须由产品先拍板。
- `rootItemId` 在链路起点推荐写为自身 `id`，实现时需要决定是预生成 `DayPlanItem.id`，还是创建后再补写一次。
- 删除某一天的延续实例后，次日是否继续自动补下一条，当前按“继续”推荐，但仍建议最终确认。

### 验证结果
- 本轮仅修改文档，未运行构建与 lint。

## 2026-04-26（V2-7：临时重复 Todo 最小可用版本）

### 本轮目标
- 实现临时重复 Todo 的最小可用版本
- 允许修改数据模型、schema、存储兼容与相关 UI
- 不做分次跨天延续、进度继承与大规模 UI polish
...
### 本轮关键判断
- 当前最稳的当天实例写法是直接创建 `source = 'auto_generated'` 的当天 `DayPlanItem`，并带上新模板的 `templateId + targetDate`，这样后续同步会复用同一条实例而不会重复生成。
- `templateKind` 是当前最小且稳定的模板区分方案，比 `source` 或布尔标记更适合后续维护。
- `activityTypeId` 若继续强绑到 `todo_recurring`，会逼出“隐藏种草分类”这种脏方案，因此本轮直接放宽为对 `todo_recurring` 可空。

### 本轮关键决策
- 新增 `templateKind = 'grass' | 'todo_recurring'`。
- 历史模板默认兼容为 `grass`。
- `grass` 仍要求 `activityTypeId` 必填；`todo_recurring` 允许为空。
- Todo 添加区新增重复规则 select：
  - `none`
  - `daily`
  - `weekly`
  - `monthly`
...
- 选择非 `none` 时：
  - 创建后台托管 `TaskTemplate`
  - 当天立即创建一条 `auto_generated` 实例
  - 后续日期继续由自动生成链路接管
- `todo_recurring` 模板不进入：
  - 种草列表
  - 管理种草
  - 拔草推荐
  - 从种草添加
- 本轮补了最小“停止重复”入口：
  - 仅对 `todo_recurring` 自动实例可见
  - 点击后归档后台模板

### 本轮修改
- 更新 `src/types/models.ts`
  - 增加 `templateKind`
  - 放宽 `TaskTemplate.activityTypeId`
- 更新 `src/db/schema.ts`
  - 为旧数据缺少 `templateKind` 提供默认 `grass`
  - 按 `templateKind` 区分 `activityTypeId` 约束
- 更新 `src/db/storage.ts`
  - `TaskTemplate` 归一化默认写入 `templateKind = 'grass'`
- 更新 `src/mocks/app-data.ts`
  - 现有模板补 `templateKind = 'grass'`
- 更新 `src/features/todo/TodoModePanel.tsx`
  - Todo 添加区加入重复规则选择
  - 创建 `todo_recurring` 模板与当天实例
  - 放开 `todo_recurring` 自动实例的完成、取消完成与当天删除
  - 新增最小“停止重复”入口
- 更新 `src/features/decision/recommendation.ts`
  - 仅允许 `grass` 模板进入推荐候选
- 更新 `src/features/templates/CreateTaskTemplateForm.tsx`
  - 普通种草显式写入 `templateKind = 'grass'`
...
### 当前风险与待确认问题
- 当前时段锚定仍依赖 scene tag 精确命名匹配；若用户初始化数据里没有“上午 / 下午 / 白天 / 晚上 / 夜间”这类 tag，未来重复实例可能退回默认白天。
- 当前最小“停止重复”入口只作用于后台模板，不额外弹出独立管理面板；若后续重复 Todo 数量变多，可能仍需要单独管理页。
- 非分次重复实例的完成 / 取消完成本轮已补 `RecurringTaskInstance` 同步，但历史旧实例若已存在不一致状态，不做额外修复。
...
## 2026-04-26（临时重复 Todo 规则与建模方案）

### 本轮目标
- 只补临时重复 Todo 的产品规则、数据建模与实现拆分方案
- 不改源码实现
- 允许同步规则文档、模型文档与交接文档
...
### 本轮关键判断
- 临时重复 Todo 不能直接塞进 `DayPlanItem`，否则会形成第二套重复系统，并与现有 `TaskTemplate + RecurringTaskInstance + auto_generated` 链路分叉。
- 用户希望从 Todo 主入口直接创建重复事项，这说明“重复”应从交互入口下沉到 Todo，但不代表底层应放弃模板层。
- 若直接复用种草模板而不加区分，会污染“种草”心智，因此必须补一层模板种类语义。

### 本轮关键决策
- 推荐采用“模板中心化”方案，并将其定义为：
  - 用户从 Todo 区创建重复事项
  - 系统创建后台托管的 Todo 重复模板
  - 未来重复继续走现有自动生成链路
- 推荐新增 `TaskTemplate.templateKind`：
  - `grass`
  - `todo_recurring`
- 推荐后续将 `activityTypeId` 调整为对 `todo_recurring` 可为空，避免伪造种草分类。
- `todo_recurring` 模板不应：
  - 出现在普通种草列表
  - 参与拔草推荐
  - 参与“从种草添加”
- 删除某一天实例不应影响未来重复；停用整条重复 Todo 应作用于后台模板。

### 本轮修改
- 更新 `product-rules.md`
  - 补充临时重复 Todo 的产品规则草案
- 更新 `app-structure.md`
  - 补充 Todo 入口重复规则与种草区隔离边界
- 更新 `data-model.md`
  - 补充 `templateKind` 方向、兼容策略与过滤点
- 更新 `task-list.md`
  - 新增 V2-7：临时重复 Todo 建模与接入
- 更新 `handoff.md`
  - 同步当前下一阶段重点为临时重复 Todo 方案落地
- 更新 `dev-log.md`
  - 记录本轮规则与建模判断
...
### 当前风险与待确认问题
- 当前文档方案已推荐 `templateKind` 路线，但“重复 Todo 的最小停用入口”仍需产品确认是放在 Todo 条目侧、设置页还是独立面板。
- 若后续不愿调整 `activityTypeId` 为可空，就需要额外设计隐藏分类方案，这会增加实现复杂度和心智污染风险。
- Todo 重复模板的时段锚定，后续仍需确认是继续借用 `sceneTagIds`，还是单独引入更直接的时段字段。

### 验证结果
- 本轮仅修改文档，未运行构建与 lint。

## 2026-04-26（V2 试用修复 1：Todo 基础交互与低风险 UI 修复）

### 本轮目标
- 修复 Todo 条目的基础交互缺口
- 补齐低风险的实例层编辑、删除、取消完成与分次推进体验
- 收口完成态、准备备注与分次进度条展示
- 不改数据模型、不改 schema、不改自动生成规则
...
### 本轮关键判断
- 这批试用问题里，完成态视觉、准备备注展示、标题编辑、`decision_selected` 删除与临时分次推进都属于当前实例层即可闭环的改动。
- 临时重复 Todo、分次跨天延续、进度继承会碰模板层和自动生成链路，本轮必须刻意不混入。
- 取消完成需要控制在低风险范围内，因此本轮只放开非分次条目的普通取消完成，并继续避免触碰自动生成回补逻辑。

### 本轮关键决策
- Todo 标题编辑只落在 `DayPlanItem.title`，不新增正文，不回写 `TaskTemplate.title`。
- `manual_temporary` 与 `decision_selected` 允许编辑和删除当天实例；`auto_generated` 继续不提供编辑和删除。
- 完成态通过条目级 class 实现标题删除线与内容降透明，但保留 checkbox 可见度。
- 准备备注改为直接灰字直显，不再通过“查看准备”按钮折叠。
- 临时分次事项放开推进，但仍只更新当前 `DayPlanItem`；没有 `recurringInstanceId` 时不补周期实例同步。
- 取消完成本轮仅对非分次的 `manual_temporary` 和 `decision_selected` 生效；已完成的 `auto_generated` 继续不支持取消，避免引入周期实例同步风险。

### 本轮修改
- 更新 `src/features/todo/TodoModePanel.tsx`
  - 为 `manual_temporary` 和 `decision_selected` 增加标题编辑态、保存与取消
  - 放开 `decision_selected` 删除当天实例
  - 为非分次条目增加取消完成
  - 删除准备备注折叠按钮，改为直显
  - 放开临时分次事项推进
  - 为分次事项补可视化进度条和百分比摘要
  - 为完成态补条目级 class
- 更新 `src/styles/globals.css`
  - 补齐 Todo 完成态、标题编辑态、删除按钮弱化与分次进度条样式
- 更新 `manual-test-checklist.md`
  - 增加标题编辑、取消完成、`decision_selected` 删除、完成态视觉与临时分次推进的手测点
- 更新 `handoff.md`
  - 同步本轮新增能力、边界与仍未处理项
- 更新 `dev-log.md`
  - 记录本轮判断、决策、修改与风险
...
- 未改 schema
- 未改自动生成规则
- 未放开 `auto_generated` 删除
- 未做拖拽排序、导出、搜索
- 未引入新依赖

### 当前风险与待确认问题
- 非分次 `auto_generated` 条目本轮仍不支持取消完成，因为当前完成逻辑本身没有同步其 `RecurringTaskInstance`，若贸然回滚容易引入实例层和周期层不一致。
- 分次推进仍沿用现有 `Math.max` 逻辑，因此本轮只是放开临时事项推进，并未允许进度回退。
- `decision_selected` 标题编辑只影响当天实例，这符合本轮边界，但后续若要把实例改名同步回种草，需要单独补规则。
