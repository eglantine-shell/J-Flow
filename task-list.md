...
说明：
- 以下分档用于指导“网页端还值不值得继续做”的排期判断
- 目标不是继续扩功能面，而是评估 App 化前最小补齐范围
...
## A2. 生命周期与 recurrence 手动验证补强

### 目标
- 在 App 化前，先把最容易误解的行为边界测透

### 最小要求
- 补齐 repeating occurrence 删除 / 停止重复 / 顺延并存验证
- 补齐种草 `active / picked / archived` 转换验证
- 补齐删除来自种草的 Todo 与“是否回库”的边界验证

### 原因
- 当前规则已经复杂到值得单独做回归闭环
- 这比继续加新功能更能提升后续 App 设计确定性
...
### 最小要求
- 最小搜索或按状态 / 时间块 / 来源的轻量筛选其一

### 原因
- 从完整 Todo List 产品视角，这属于自然需求
- 但它是否必须在网页端补，取决于网页版是否还会继续真实试用
...
### V2.3
- 若网页端继续试用：补轻量 Todo 搜索 / 筛选
- 若准备冻结网页：跳过实现，进入 App 设计准备
