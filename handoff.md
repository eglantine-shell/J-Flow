...
## 最近一次完成的 task
- 设置页 + 测试专用重置入口
- `J-Flow` 旁已补上轻量设置图标入口，并新增受初始化守卫保护的 `/settings` 页面
- 设置页已补齐：
  - 时间场景新增 / 删除
  - 活动类型新增 / 删除
  - 推荐并列时的加入时间排序方向设置
  - “重置应用（测试用）”入口
- 删除时间场景时会同步从所有模板中移除该标签，不删除模板本身，也不删除历史实例
- 删除活动类型时会先检查是否仍被任一模板使用；若仍在使用则阻止删除
- 测试重置会清空当前本地应用数据，并回到第一次打开应用的初始化流程

## 下一步最推荐做什么
- 先完整走一遍“初始化 → 首页录入 → 决策推荐 → Todo → 设置删除 / 排序 / 测试重置 → 再初始化”的真机试用流程，重点确认设置页与重置入口是否顺手。
- 若继续做产品收口，可再评估是否补 README 中的设置页说明，以及是否开始最小业务测试。
