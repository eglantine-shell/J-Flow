...
## 最近一次完成的 task
- 文档对齐 + 模板添加表单返工（仅 UI / 交互修正）
- README 已同步当前主路径、模板日期条件显隐，以及模板添加区已收敛为单区块的现状
- 模板添加表单顺序已按“活动类型 → 内容 → 时间场景 → 兴趣程度 → 必要事项+重复规则 → 条件日期 → 需要准备+分次事项 → 条件备注”调整
- 活动类型与时间场景的 inline creator 已补上明确可点击的保存动作，并保留 Enter 作为辅助
- 手机端已恢复“必要事项 + 重复规则”与“需要准备 + 分次事项”的双列并排，同时统一了 5 块控件的视觉高度节奏

## 下一步最推荐做什么
- 先在手机上真实试用新的模板添加面板，重点看新的保存按钮、双列并排和控件高度是否顺手。
- 若继续做 UI，可再压缩模板管理展开态，但不要顺手扩到设置页或首页结构。
- 工程侧已补齐 GitHub Pages 部署配置，可直接进入仓库级部署验证。

## 当前已知边界 / 技术债
- 暂无自动化业务测试，当前主要依赖 `npm run build` 与 `npm run lint`。
- 推荐、Todo、模板管理的交互都已可用；模板添加表单已完成本轮顺序、保存交互和移动端并排修正，但模板管理展开态和推荐面板仍有进一步压缩空间。
