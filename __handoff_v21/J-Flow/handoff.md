...
## 最新状态（2026-05-23，Windows V2.1 portable 图标修复，优先参考）
- 用户手测确认 V2.1 exe 功能正常；本轮修复 Windows 标题栏/任务栏图标仍显示 Electron 的问题。
- 当前最终产物为：
  - `E:\J-Flow\release\J-Flow-V2.1-win-portable.exe`
  - `E:\J-Flow\_mac_import\J-Flow\release\J-Flow-V2.1-win-portable.exe`
  - `E:\J-Flow\_mac_import\J-Flow\release-win-v21-icon2\J-Flow-V2.1-win-portable.exe`
- 产物大小：
  - `94,125,575 bytes`
- 产物 SHA256：
...
- 图标链路当前处理方式：
  ...
  - `electron/main.ts` 在 Windows 下对 `BrowserWindow` 显式传入该图标，并设置 `appUserModelId`
- 当前仍保持 `win.signAndEditExecutable=false`，因为本机启用后会因 `winCodeSign` 解压 symlink 权限失败。
- 已验证：外层 portable exe 关联图标为 J-Flow 圆角图标，`win-unpacked/resources/icon.ico` 存在，asar 中 V2.1 功能标记仍存在。
- 仍需用户重新打开最终 exe，确认标题栏小图标与任务栏图标均已刷新；Windows 图标缓存可能需要关闭旧进程或重启资源管理器后才刷新。
- 文档口径提醒：不要把“本地文件夹同步”写成官方云同步。
## 最新状态（2026-05-23，Windows V2.1 portable 已用 Mac 新源码重打包，优先参考）
- 当前有效源码来自用户从 MacBook 打包迁移过来的完整工作区：
...
- 产物 asar 已确认包含：
  ...
  - `localFolder`
  - `maybeAutoSync`
- 当前仍需用户打开新版 exe 做功能级手测。
- Windows 下 SQLite 相关测试存在临时文件 `EBUSY` 清理风险，业务构建与打包已通过。
- 已补充复验标准 `pnpm run build:desktop`：清理迁移遗留的旧 `dist-desktop/renderer` 与 `dist-electron` 生成目录后，完整脚本通过。
- 最终打包采用隔离输出目录 `release-win-v21-final`，用于避开默认 `release` 目录旧 exe 写入占用。
- 文档口径提醒：不要把“本地文件夹同步”写成官方云同步。
